...
class ThreadBlockQueue
{
  ...
  public:
    ...
    // Create safe queue.
    ThreadBlockQueue() = default;
    ThreadBlockQueue (ThreadBlockQueue&& sq)
    {
        m_queue = std::move (sq.m_queue);
    }

    ThreadBlockQueue (const ThreadBlockQueue& sq)
    {
    ...
    ThreadBlockQueue& operator= (const ThreadQueue& sq)
    {
        if (this != &sq)
        {
            std::lock_guard<std::mutex> lock1 (m_mutex);
    ...
    ThreadBlockQueue& operator= (ThreadQueue && sq)
    {
        std::lock_guard<std::mutex> lock (m_mutex);
        m_queue = std::move (sq.m_queue);
...
void swap (ThreadBlockQueue<T, Container>& q1, ThreadBlockQueue<T, Container>& q2)
{
    q1.swap (q2);
}
